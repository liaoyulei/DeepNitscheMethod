...
class resnet_nitsche:
    def __init__(self, dimension, layer, unit, activation):
        ...
        self.loss_b = 2 * dimension * tf.reduce_mean(self.beta / 2 * (output_b - self.g(self.x_b)) ** 2 + (self.g(self.x_b) - output_b) * self.norm2_grad(output_b, self.x_b) ** (self.p/2 - 1) * self.diff_n(output_b, self.x_b))
#        self.loss_b = 2 * dimension * tf.reduce_mean(self.beta / 2 * output_b ** 2 - output_b * self.diff_n(output_b, self.x_b) - self.g(self.x_b) * (self.beta * output_b - self.diff_n(output_b, self.x_b)))
        self.loss_i = tf.reduce_mean(self.norm2_grad(output_i, self.x_i) ** (self.p/2) / self.p - self.f(self.x_i) * output_i)
    ...
    def u(self, x):
        return (self.p - 1) / self.p * (1/2) ** (1 / (self.p - 1)) * (1 - tf.reduce_sum(x ** 2, axis = 1) ** (self.p / (self.p - 1) / 2))
    #    return tf.reduce_sum(x ** 2, axis = 1) ** ((self.p - 2) / (self.p - 1) / 2)
