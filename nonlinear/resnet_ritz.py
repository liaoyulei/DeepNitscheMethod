...
class resnet_possion:
    def __init__(self, dimension, layer, unit, activation):
        ...
        self.loss_i = tf.reduce_mean(self.norm2_grad(output_i, self.x_i) ** (self.p/2) / self.p + output_i ** 2 / 2 - self.f(self.x_i) * output_i)
        ...
        self.berrl2 = 2 * self.l2(output_b - self.u(self.x_b), self.x_b)
        self.berrh1 = 2 * tf.sqrt(tf.reduce_mean(self.norm2_grad(output_b - self.u(self.x_b), self.x_b)))
        ...
        tf.compat.v1.summary.scalar('berrl2', self.berrl2)
        tf.compat.v1.summary.scalar('berrh1', self.berrh1)
    ...
    def f(self, x):
        return tf.sin(np.pi*x[:, 0])*tf.sin(np.pi*x[:, 1]) + 2*np.pi**2*tf.sin(np.pi*x[:, 0])*tf.sin(np.pi*x[:, 1])*(np.pi**2*tf.cos(np.pi*x[:, 0])**2*tf.sin(np.pi*x[:, 1])**2 + np.pi**2*tf.cos(np.pi*x[:, 1])**2*tf.sin(np.pi*x[:, 0])**2)**(self.p/2 - 1) - np.pi*tf.cos(np.pi*x[:, 0])*tf.sin(np.pi*x[:, 1])*(self.p/2 - 1)*(np.pi**2*tf.cos(np.pi*x[:, 0])**2*tf.sin(np.pi*x[:, 1])**2 + np.pi**2*tf.cos(np.pi*x[:, 1])**2*tf.sin(np.pi*x[:, 0])**2)**(self.p/2 - 2)*(2*np.pi**3*tf.cos(np.pi*x[:, 0])*tf.cos(np.pi*x[:, 1])**2*tf.sin(np.pi*x[:, 0]) - 2*np.pi**3*tf.cos(np.pi*x[:, 0])*tf.sin(np.pi*x[:, 0])*tf.sin(np.pi*x[:, 1])**2) - np.pi*tf.cos(np.pi*x[:, 1])*tf.sin(np.pi*x[:, 0])*(self.p/2 - 1)*(np.pi**2*tf.cos(np.pi*x[:, 0])**2*tf.sin(np.pi*x[:, 1])**2 + np.pi**2*tf.cos(np.pi*x[:, 1])**2*tf.sin(np.pi*x[:, 0])**2)**(self.p/2 - 2)*(2*np.pi**3*tf.cos(np.pi*x[:, 0])**2*tf.cos(np.pi*x[:, 1])*tf.sin(np.pi*x[:, 1]) - 2*np.pi**3*tf.cos(np.pi*x[:, 1])*tf.sin(np.pi*x[:, 0])**2*tf.sin(np.pi*x[:, 1]))
...
net = resnet_possion(2, 5, 10, 'tanh') #d-Dimention l-Layers u-Units
