...
class resnet_nitsche:
    def __init__(self, dimension, layer, unit, activation):
        ...
        self.loss_b = self.dimension * (tf.reduce_mean(self.beta / 2 * output_d ** 2 - output_d * self.agradn(output_d, self.x_d) - self.g_d(self.x_d) * (self.beta * output_d - self.agradn(output_d, self.x_d))) - tf.reduce_mean(self.g_n(self.x_n) * output_n))
    ...
    def train(self, sess):
        ...
        sess.run(self.opt, feed_dict = {self.x_d: x_d, self.x_n: x_n, self.x_i: x_i})
