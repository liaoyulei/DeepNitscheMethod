...
class resnet_nitsche:
    def __init__(self, dimension, layer, unit, activation):
        ...
        self.loss_b = 2 * tf.reduce_mean(beta / 2 * output_d ** 2 - output_d * self.diff_n(output_d, self.x_d) - self.g_d(self.x_d) * (beta * output_d - self.diff_n(output_d, self.x_d))) - 2 * tf.reduce_mean(self.g_n(self.x_n) * output_n)
    ...
    def diff_n(self, u, x): #x_d
        grad = tf.gradients(u, x)[0]
        difflist = []
        difflist.append(-grad[:self.batch_d, 1])
        difflist.append(grad[self.batch_d:, 1])
