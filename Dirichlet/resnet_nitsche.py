...
class resnet_nitsche:
    def __init__(self, dimension, layer, unit, activation):
        ...
        self.loss_b = 2 * dimension * tf.reduce_mean(beta / 2 * output_b ** 2 - output_b * self.diff_n(output_b, self.x_b) - self.g(self.x_b) * (beta * output_b - self.diff_n(output_b, self.x_b)))
