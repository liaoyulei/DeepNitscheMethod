...
class resnet_nitsche:
    def __init__(self, dimension, layer, unit, activation):
        ...
        self.loss_b = self.dimension * (tf.reduce_mean(beta / 2 * output_d ** 2 - output_d * self.agradn(output_d, self.x_d) - self.g_d(self.x_d) * (beta * output_d - self.agradn(output_d, self.x_d))) - tf.reduce_mean(self.g_n(self.x_n) * output_n))
