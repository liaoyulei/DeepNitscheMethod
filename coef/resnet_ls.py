...
class resnet_ls:
    def __init__(self, dimension, layer, unit, activation):
        ...
        self.loss_b = self.dimension * (tf.reduce_mean((output_d - self.g_d(self.x_d)) ** 2) + tf.reduce_mean((self.agradn(output_n, self.x_n) - self.g_n(self.x_n)) ** 2))
        self.loss_i = tf.reduce_mean((self.divagrad(output_i, self.x_i) + self.f(self.x_i)) ** 2)
    ...
    def divagrad(self, u, x):
        grad = tf.gradients(u, x)[0]
        w1 = self.a11(x) * grad[:, 0] + self.a12(x) * grad[:, 1]
        w2 = (self.a21(x) * grad[:, 0] + self.a22(x) * grad[:, 1])
        w1grad = tf.gradients(w1, x)[0]
        w2grad = tf.gradients(w2, x)[0]
        return w1grad[:, 0] + w2grad[:, 1]

    def agradn(self, u, x): #x_n
        grad = tf.gradients(u, x)[0]  
        difflist = []
        difflist.append(-self.a11(x[: self.batch_d, :]) * grad[: self.batch_d, 0] - self.a12(x[: self.batch_d, :]) * grad[: self.batch_d, 1])
        difflist.append(-self.a21(x[self.batch_d: , :]) * grad[self.batch_d: , 0] - self.a22(x[self.batch_d: , :]) * grad[self.batch_d: , 1])
